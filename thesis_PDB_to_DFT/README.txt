...
(1) Open get_translated.py and select the methionine and aromatic residue position numbers
    corresponding to a Met-aromatic interaction by manually editing AROMATIC_RESIDUE and METHIONINE_RESIDUE.
    Run the script. This will place all the .xyz files into ~/thesis_PDB_to_DFT/orca_workdir
(2) Open the .xyz files in Avogadro and manually add hydrogens. Save the files. Yes... I know this is frustrating.
(3) Next we generate input decks using xyz_to_inp.py. Simply execute the script.
(4) Then we generate the shell script for running ORCA calculations using inp_to_out.py. 
    Simply execute the script. The shell script inp_to_out.sh will be located in ~/thesis_PDB_to_DFT/orca_workdir
(5) Run the shell script. To do so, open up cmd or terminal, depending on OS, then cd into orca_workdir.
    For me, it is:
    	$ cd ~/Desktop/thesis_PDB_to_DFT/orca_workdir
    Then type:
	    $ sh inp_to_out.sh
    And ENTER. The DFT calculation will now begin.
    I used the default DFT parameters which render the DFT calculations very quick. Be advised that the amount of
    time taken to complete the DFT will increase significantly if my code is modified with other functionals. Increase
    your fan speeds if you do change my functionals as your computer may get super hot.
(6) If all goes well, ~/thesis_PDB_to_DFT/orca_workdir will be loaded with many .out files. Use cleanup.py to clean out
...
(7) Use get_energies.py to generate a plot for the translation series. The x-axis may need to be flipped.
(8) I highly recommend completely clearing ~/thesis_PDB_to_DFT/orca_workdir as soon as the experiment is done.
